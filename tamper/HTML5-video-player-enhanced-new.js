...
const mediaCore = (function () {
        ...
        const mediaPlusBaseApi = {
            ...
            set(keyName, val) {
                if (
                    ...
                    return originDescriptors[keyName].set.apply(mediaElement, [
                        val,
                    ])
                }
            },
        ...
        extApiKeys.forEach((key) => {
            baseApiKeys.forEach((baseKey) => {
                ...
                mediaPlusApi[`${baseKey}${firstUpperCase(key)}`] = function () {
                    return mediaPlusBaseApi[baseKey].apply(null, [
                        key,
                        ...arguments,
                    ])
                }
    ...
    function hijackPrototypeProperty(element, property) {
        ...
        original.Object.defineProperty.call(
            Object,
            element.prototype,
            property,
            {
                ...
                enumerable: true,
                get: function () {
                    const val = originDescriptors[property].get.apply(
                        this,
                        arguments,
                    )
                ...
                set: function (value) {
                    ...
                    if (
                        ['playbackRate', 'volume', 'currentTime'].includes(
                            property,
                        )
                    ) {
                        const mediaPlusApi = createMediaPlusApi(this)
                    ...
                    return originDescriptors[property].set.apply(
                        this,
                        arguments,
                    )
                },
...
function isRegisterKey(event) {
    ...
    return (
        hasUseKey.keyCodeList.includes(keyCode) ||
...
function isVideoElement(element) {
    return (
        element &&
        (element instanceof HTMLVideoElement || element.HTMLVideoElement)
    )
}
...
function isAudioElement(element) {
    return (
        element &&
        (element instanceof HTMLAudioElement || element.HTMLAudioElement)
    )
}
...
function isLocalStorageUsable() {
    return (
        window.localStorage &&
        window.localStorage.getItem instanceof Function &&
...
function isGlobalStorageUsable() {
    return (
        ...
        window.GM_listValues instanceof Function
    )
}
    ...
    localStorageApis.forEach((apiKey) => {
        if (isLocalStorageUsable()) {
            ...
            rawLocalStorage[apiKey] = function () {
                return rawLocalStorage[`_${apiKey}_`].apply(
                    localStorage,
                    arguments,
                )
            }
    ...
    getConfPathList(config) {
        ...
        function getConfPathList(config, path = '') {
            Object.keys(config).forEach((key) => {
                ...
                if (
                    Object.prototype.toString.call(config[key]) ===
                    '[object Object]'
                ) {
        ...
        if (isLocalStorageUsable()) {
            ...
            if (localConf !== null && localConf !== undefined) {
                try {
                    ...
                    console.error(
                        'configManager parse localStorage error:',
                        key,
                        localConf,
                    )
                }
    ...
    setLocalStorage(confPath, val) {
        ...
        if (isLocalStorageUsable()) {
            try {
                ...
                console.error(
                    'configManager set localStorage error:',
                    key,
                    val,
                    e,
                )
    ...
    setGlobalStorage(confPath, val) {
        ...
        if (isGlobalStorageUsable()) {
            try {
                ...
                console.error(
                    'configManager set globalStorage error:',
                    key,
                    val,
                    e,
                )
...
async function initUiConfigManager() {
    const isUiConfigPage =
        location.href.indexOf('h5player.anzz.top/tools/json-editor') > -1 ||
        location.href.indexOf('ankvps.gitee.io/h5player/tools/json-editor') > -1
    const isUiConfigMode =
        location.href.indexOf('saveHandlerName=saveH5PlayerConfig') > -1
...
let TCC$1 = class TCC {
    constructor(taskConf, doTaskFunc) {
        ...
        this.doTaskFunc =
            doTaskFunc instanceof Function ? doTaskFunc : function () {}
    }

    setTaskConf(taskConf) {
        this.conf = taskConf
    }
...
class Debug {
    ...
    createDebugMethod(name) {
        ...
        return function () {
            ...
            const arg0 = arg[0]
            arg.unshift(
                `color: ${textColor}; background-color: ${bgColor}; ${customStyle}`,
            )
            ...
            if (trace) {
                if (traceGroup) {
                    const arg1Str =
                        typeof arg0 === 'string'
                            ? arg0
                            : Object.prototype.toString.call(arg0)
                    console.groupCollapsed(
                        `%c ${timeStr}${msg} ${arg1Str}`,
                        `color: ${textColor}; background-color: ${bgColor}; ${customStyle}`,
                    )
...
const taskConf = {
    ...
    'youtube.com': {
        init: function (h5Player, taskConf) {
            if (h5Player.hasBindSkipAdEvents) {
                return
            }
            ...
            setInterval(function () {
                ...
                const adSkipBtnModern = document.querySelector(
        ...
        shortcuts: {
            ...
            callback: function (h5Player, taskConf, data) {
                ...
                if (event.keyCode === 27) {
                    /* 取消播放下一个推荐的视频 */
                    if (
                        document.querySelector('.ytp-upnext').style.display !==
                        'none'
                    ) {
                        document
                            .querySelector('.ytp-upnext-cancel-button')
                            .click()
                    }
                }
            },
    ...
    'bilibili.com': {
        ...
        webFullScreen: function () {
            ...
            if (
                oldWebFullscreen ||
                (webFullscreenEnter && webFullscreenLeave)
            ) {
                const webFullscreen =
                    oldWebFullscreen ||
                ...
                if (event.keyCode === 27) {
                    ...
                    if (
                        ...
                        const webFullscreenLeave =
                            ...
                            $q('.squirtle-pagefullscreen-active')
                        if (
                            getComputedStyle(webFullscreenLeave).display !==
                            'none'
                        ) {
    ...
    'ixigua.com': {
        ...
        webFullScreen: [
            'xg-cssfullscreen.xgplayer-cssfullscreen',
            '.xgplayer-control-item__entry[aria-label="剧场模式"]',
            '.xgplayer-control-item__entry[aria-label="退出剧场模式"]',
        ],
    },
    'tv.sohu.com': {
        ...
        shortcuts: {
            ...
            callback: function (h5Player, taskConf, data) {
                ...
                if (!h5Player[keyName]) {
                    ...
                    if (
                        h5Player[keyName] === h5Player.playbackRate ||
                        h5Player[keyName] === true
                    ) {
                        if (
                            window.sessionStorage.playbackRate &&
                            /(c|x|z|1|2|3|4)/.test(key)
                        ) {
                            const curSpeed = Number(
                                window.sessionStorage.playbackRate,
                            )
                            const perSpeed =
                                curSpeed - 0.1 >= 0 ? curSpeed - 0.1 : 0.1
                            const nextSpeed =
                                curSpeed + 0.1 <= 4 ? curSpeed + 0.1 : 4
        ...
        shortcuts: {
            ...
            callback: function (h5Player, taskConf, data) {
                eachParentNode(h5Player.player(), function (parentNode) {
                    if (
                        parentNode.getAttribute('data-fullscreen-container') ===
                        'true'
                    ) {
                        const goBackBtn =
                            parentNode.parentNode.querySelector('div>a>i>u')
                        if (goBackBtn) {
        ...
        init: function (h5Player, taskConf) {
            ...
            const setVideoTitle = () => {
                if (wrapEl && wrapEl.querySelector('.video-info-detail')) {
                    ...
                    const accountNameEL =
                        videoInfo.querySelector('.account-name')
                    /* 移除accountName前面的@符号 */
                    ...
                    const title = `${titleText} - ${accountName}`.replace(
                        /[\\/:*?"<>|]/g,
                        '-',
                    )
    ...
    'zhihu.com': {
        ...
        play: function (h5Player, taskConf, data) {
            ...
            if (player && player.parentNode && player.parentNode.parentNode) {
                const maskWrap = player.parentNode.parentNode.querySelector(
                    'div~div:nth-child(3)',
                )
                if (maskWrap) {
    ...
    'twitter.com': {
        init: function (h5Player, taskConf) {
            ...
            const setVideoTitle = () => {
                if (
                    wrapEl &&
                    !wrapEl.getAttribute('data-title') &&
                    wrapEl.querySelector('div[data-testid="tweetText"]')
                ) {
    ...
    getContainer() {
        ...
        t.eachParentNode(d, function (parentNode) {
            ...
            const isInsideTheBox =
                parentBox.width <= domBox.width &&
                parentBox.height <= domBox.height
            if (isInsideTheBox) {
                container = parentNode
            } else {
                return true
                ...
                (event) => {
                    t.observeList.forEach((target) => {
                        const isVisibility =
                            ...
                            target.IntersectionObserverEntry.intersectionRatio >
                                0
                        const isReg =
                            target.MouseObserverEvent[event.type] === true
...
class I18n {
    ...
    t(path) {
        ...
        if (!result && t._locale !== t._defaultLanguage) {
            result = t.getValByPath(
                t._languages[t._defaultLanguage] || {},
                path,
            )
        }
    ...
    isMatchCurLang(lang) {
        ...
        return (
            lang.includes(curLang) ||
            lang.includes(curLang2) ||
            lang.includes(curLang3) ||
            lang.includes(curLang4)
        )
    }
...
var zhCN = {
    ...
    mediaDownload: {
        ...
        notFoundMediaSource:
            '未找到对应的媒体流数据，数据可能被清理或者媒体元素已经被移除，建议刷新页面后重试',
    },
    menu: '菜单',
    more: '更多',
    moreActions: '更多操作',
...
var enUS = {
    ...
    experimentFeaturesWarning:
        'Experimental features are likely to cause some uncertain problems, please turn on with caution',
    useMediaDownloadTips:
        'To use the download capability, you need to enable experimental features.\nExperimental features can easily cause some uncertain problems. Are you sure you want to enable it?',
    allowExternalCustomConfiguration:
        'Enable external customization capabilities',
    notAllowExternalCustomConfiguration:
        'Turn off external customization capabilities',
    ...
    mediaDownload: {
        ...
        downloading:
            'The file is being downloaded. Are you sure you want to execute this operation again?',
        hasDownload:
            'The file has been downloaded. Are you sure you want to execute this operation again?',
        confirmTitle: 'Please enter the file name',
        notSupport:
            'The current media file cannot be downloaded. The download function needs to be optimized and improved',
        notEndOfStream:
            'The media data is not fully ready, are you sure you want to download it?',
        cancelAutoDownload: 'Cancel automatic download?',
        autoDownload:
            'The media data is not fully ready, do you want to automatically download it when it is ready?',
        notFoundMediaSource:
            'The corresponding media stream data was not found, the data may have been cleared or the media element has been removed, it is recommended to refresh the page and try again',
    },
    menu: 'Menu',
    more: 'More',
    ...
    tipsMsg: {
        ...
        playbackrestoreoff:
            'The function of restoring the playback progress is disabled. Press SHIFT+R to turn on the function',
...
var ru = {
    ...
    disableCurrentInstanceGUI:
        'отключить текущий графический интерфейс пользователя',
    disableGUITemporarily:
        'Временно отключить графический интерфейс пользователя',
    ...
    enableInitAutoPlay:
        'Разрешить автоматическое воспроизведение видео на этом сайте',
    restoreConfiguration: 'Восстановить глобальную конфигурацию по умолчанию',
    blockSetPlaybackRate:
        'Отключить логику регулирования скорости по умолчанию',
    blockSetCurrentTime:
        'Отключить логику управления ходом воспроизведения по умолчанию',
    blockSetVolume: 'Отключить логику управления громкостью по умолчанию',
    unblockSetPlaybackRate:
        'Разрешить логику регулировки скорости по умолчанию',
    unblockSetCurrentTime:
        'Разрешить логику управления ходом воспроизведения по умолчанию',
    ...
    experimentFeaturesWarning:
        'Экспериментальные функции могут вызвать определенные проблемы, включайте их с осторожностью.',
    useMediaDownloadTips:
        'Чтобы использовать функцию загрузки, вам необходимо включить экспериментальную функцию.\nЭкспериментальные функции могут легко вызвать некоторые неопределенные проблемы. Вы уверены, что хотите включить ее?',
    ...
    openCustomConfigurationEditor:
        'Открыть редактор пользовательской конфигурации',
    ...
    mediaDownload: {
        ...
        hasDownload:
            'Файл скачан. Вы уверены, что хотите повторить эту операцию?',
        confirmTitle: 'Пожалуйста, введите имя файла',
        notSupport:
            'Текущий медиафайл невозможно загрузить, а функцию загрузки необходимо оптимизировать и улучшить.',
        notEndOfStream:
            'Медиаданные еще не полностью готовы. Вы уверены, что хотите их скачать?',
        cancelAutoDownload: 'Отменить автоматическую загрузку?',
        autoDownload:
            'Будут ли медиаданные загружаться автоматически после их полной готовности?',
        notFoundMediaSource:
            'Соответствующие данные медиапотока не найдены. Возможно, данные были очищены или медиа-элементы удалены. Рекомендуется обновить страницу и повторить попытку.',
    },
    menu: 'Меню',
    more: 'Больше',
    ...
    comingSoon:
        'Больше функций находится в процессе улучшения, следите за обновлениями',
    ffmpegScript: 'Скрипт слияния/преобразования аудио и видео',
    autoGotoBufferedTime: 'Автоматически перейти к времени буфера',
    disableAutoGotoBufferedTime:
        'Отключить автоматический переход к времени буфера',
    mouse: {
    ...
    tipsMsg: {
        ...
        playbackrestoreoff:
            'Функция восстановления прогресса воспроизведения отключена. Нажмите SHIFT + R, чтобы включить функцию',
...
var zhTW = {
    ...
    mediaDownload: {
        ...
        notFoundMediaSource:
            '未找到對應的媒體流數據，數據可能被清理或媒體元素已經被移除，建議刷新頁面後重試',
    },
    menu: '菜單',
    more: '更多',
    moreActions: '更多操作',
...
const monkeyMsg = {
    ...
    on: (name, fn) =>
        ...
        window.GM_addValueChangeListener(
            name,
            function (name, oldVal, newVal, remote) {
    ...
    broadcast(handler) {
        ...
        monkeyMsg._monkeyMsgBroadcastHandler_ =
            monkeyMsg._monkeyMsgBroadcastHandler_ || []
        handler instanceof Function &&
            monkeyMsg._monkeyMsgBroadcastHandler_.push(handler)
...
const crossTabCtl = {
    ...
    crossTabKeydownEvent(event) {
        ...
        if (
            t.isNeedSendCrossTabCtlEvent() &&
            isRegisterKey(event) &&
            !t.excludeShortcuts(event)
        ) {
            // 阻止事件冒泡和默认事件
...
class HookJs {
    ...
    _proxyMethodcGenerator(
        ...
        if (t.isHook(originMethod)) {
            ...
            originMethod[t.hookPropertiesKeyName] &&
            t.isHook(originMethod[t.hookPropertiesKeyName].hookMethod)
        ) {
        ...
        if (hookMethod) {
            if (!hookMethod[t.hookPropertiesKeyName].isHook) {
                ...
                util.debug.log(
                    `[hook method] ${util.toStr(parentObj)} ${methodName}`,
                )
            }
            return hookMethod
        }
        ...
        if (useProxy && Proxy) {
            ...
            keys.forEach((keyName) => {
                try {
                    ...
                    util.debug.log(
                        `[proxyMethodcGenerator] hookMethod defineProperty abnormal.  hookMethod:${methodName}, definePropertyName:${keyName}`,
                        err,
                    )
                }
            })
    ...
    _getObjKeysByRule(obj, rule) {
        ...
        if (rule === '*') {
            ...
            result = util
                .getAllKeys(obj)
                .filter((keyName) => rule.test(keyName))
        }
        ...
        if (excludeRule) {
            ...
            if (util.isReg(excludeRule)) {
                ...
                result = result.filter(
                    (keyName) => !excludeRule.includes(keyName),
                )
            } else {
    ...
    isHook(fn) {
        ...
        return (
            util.isFn(hookMethodProperties.originMethod) &&
            fn !== hookMethodProperties.originMethod
        )
    }
    ...
    hook(parentObj, hookMethods, fn, type, classHook, context, proxyHandler) {
        ...
        if (
            (!util.isRef(parentObj) && !util.isFn(parentObj)) ||
            !util.isFn(fn) ||
            !hookMethods
        ) {
            return false
        ...
        hookMethods.forEach((methodName) => {
            if (!t.isAllowHook(parentObj, methodName)) {
                util.debug.log(
                    `${util.toStr(
                        parentObj,
                    )} [${methodName}] does not support modification`,
                )
                return false
            }
            ...
            hookMethod = t._proxyMethodcGenerator(
                parentObj,
                methodName,
                originMethod,
                classHook,
            ...
            if (hookMethodProperties.classHook !== classHook) {
                util.debug.log(
                    `${util.toStr(
                        parentObj,
                    )} [${methodName}] Cannot support functions hook and classes hook at the same time `,
                )
                return false
            }
                ...
                if (fn) {
                    ...
                    for (let i = 0; i < hooks.length; i++) {
                        if (fn === hooks[i]) {
                            ...
                            util.debug.log(
                                `[unHook ${hookKeyName} func] ${util.toStr(
                                    parentObj,
                                )} ${methodName}`,
                                fn,
                            )
                    ...
                    if (Array.isArray(hookMethodProperties[hookKeyName])) {
                        hookMethodProperties[hookKeyName] = []
                        util.debug.log(
                            `[unHook all ${hookKeyName}] ${util.toStr(
                                parentObj,
                            )} ${methodName}`,
                        )
                    }
...
function hackDefineProperOnError(
    args,
    parentObj,
    methodName,
    originMethod,
...
function hackDefineProperty() {
    hookJs.before(
        Object,
        'defineProperty',
        function (args, parentObj, methodName, originMethod, execInfo, ctx) {
            const option = args[2]
            const ele = args[0]
            const key = args[1]
    ...
    build(menuOpts) {
        ...
        if (Array.isArray(menuOpts)) {
            ...
            if (Array.isArray(menuList)) {
                ...
                console.error(
                    'monkeyMenu build error, no menuList return',
                    menuOpts,
                )
            }
        }
...
const globalFunctional = {
    ...
    openAddGroupChatPage: {
        ...
        fn: () => {
            const groupChatUrl = isChinese()
                ? 'https://h5player.anzz.top/zh/home/quickStart#%E4%BA%A4%E6%B5%81%E7%BE%A4'
                : 'https://h5player.anzz.top/home/quickStart#discussion-groups'
    ...
    openCheckVersionPage: {
        ...
        fn: () => {
            ...
            if (confirm) {
                openInTab(
                    'https://greasyfork.org/zh-CN/scripts/381682/versions',
                )
            }
        },
    },
    ...
    toggleExpandedOrCollapsedStateOfMonkeyMenu: {
        title: `${
            configManager.get('enhance.unfoldMenu')
                ? i18n.t('foldMenu')
                : i18n.t('unfoldMenu')
        } 「${i18n.t('globalSetting')}」`,
        desc: `${
            configManager.get('enhance.unfoldMenu')
                ? i18n.t('foldMenu')
                : i18n.t('unfoldMenu')
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setGlobalStorage(
                    'enhance.unfoldMenu',
                    !configManager.get('enhance.unfoldMenu'),
                )
                window.location.reload()
    ...
    toggleScriptEnableState: {
        title: `${
            (configManager.get('blacklist.domains') || []).includes(
                location.host,
            )
                ? i18n.t('enableScript')
                : i18n.t('disableScript')
        } 「${i18n.t('localSetting')}」`,
        desc: `${
            (configManager.get('blacklist.domains') || []).includes(
                location.host,
        ...
        fn: () => {
            ...
            if (confirm) {
                if (isInBlacklist) {
                    configManager.setGlobalStorage(
                        'blacklist.domains',
                        blackDomainList.filter(
                            (item) => item !== location.host,
                        ),
                    )
                } else {
                    configManager.setGlobalStorage(
                        'blacklist.domains',
                        blackDomainList.concat(location.host),
                    )
                }
    ...
    toggleSetCurrentTimeFunctional: {
        ...
        title: () =>
            `${
                configManager.get('enhance.blockSetCurrentTime')
                    ? i18n.t('unblockSetCurrentTime')
                    : i18n.t('blockSetCurrentTime')
            } 「${i18n.t('localSetting')}」`,
        desc: () =>
            `${
                configManager.get('enhance.blockSetCurrentTime')
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setLocalStorage(
                    'enhance.blockSetCurrentTime',
                    !configManager.get('enhance.blockSetCurrentTime'),
                )
                window.location.reload()
            }
    ...
    toggleSetVolumeFunctional: {
        title: () =>
            `${
                configManager.get('enhance.blockSetVolume')
                    ? i18n.t('unblockSetVolume')
                    : i18n.t('blockSetVolume')
            } 「${i18n.t('localSetting')}」`,
        desc: () =>
            `${
                configManager.get('enhance.blockSetVolume')
    ...
    toggleSetPlaybackRateFunctional: {
        title: () =>
            `${
                configManager.get('enhance.blockSetPlaybackRate')
                    ? i18n.t('unblockSetPlaybackRate')
                    : i18n.t('blockSetPlaybackRate')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
            `${
                configManager.get('enhance.blockSetPlaybackRate')
    ...
    toggleAcousticGainFunctional: {
        title: () =>
            `${
                configManager.get('enhance.allowAcousticGain')
                    ? i18n.t('notAllowAcousticGain')
                    : i18n.t('allowAcousticGain')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
            `${
                configManager.get('enhance.allowAcousticGain')
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setGlobalStorage(
                    'enhance.allowAcousticGain',
                    !configManager.getGlobalStorage(
                        'enhance.allowAcousticGain',
                    ),
                )
                window.location.reload()
    ...
    toggleCrossOriginControlFunctional: {
        title: () =>
            `${
                configManager.get('enhance.allowCrossOriginControl')
                    ? i18n.t('notAllowCrossOriginControl')
                    : i18n.t('allowCrossOriginControl')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
            `${
                configManager.get('enhance.allowCrossOriginControl')
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setGlobalStorage(
                    'enhance.allowCrossOriginControl',
                    !configManager.getGlobalStorage(
                        'enhance.allowCrossOriginControl',
                    ),
                )
    ...
    toggleExperimentFeatures: {
        title: () =>
            `${
                configManager.get('enhance.allowExperimentFeatures')
                    ? i18n.t('notAllowExperimentFeatures')
                    : i18n.t('allowExperimentFeatures')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
            `${
                configManager.get('enhance.allowExperimentFeatures')
        ...
        fn: () => {
            const confirm = window.confirm(
                configManager.get('enhance.allowExperimentFeatures')
                    ? i18n.t('notAllowExperimentFeatures')
                    : i18n.t('experimentFeaturesWarning'),
            )
            if (confirm) {
                configManager.setGlobalStorage(
                    'enhance.allowExperimentFeatures',
                    !configManager.get('enhance.allowExperimentFeatures'),
                )
                window.location.reload()
            }
    ...
    toggleExternalCustomConfiguration: {
        title: () =>
            `${
                configManager.get('enhance.allowExternalCustomConfiguration')
                    ? i18n.t('notAllowExternalCustomConfiguration')
                    : i18n.t('allowExternalCustomConfiguration')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
            `${
                configManager.get('enhance.allowExternalCustomConfiguration')
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setGlobalStorage(
                    'enhance.allowExternalCustomConfiguration',
                    !configManager.getGlobalStorage(
                        'enhance.allowExternalCustomConfiguration',
                    ),
                )
    ...
    toggleDebugMode: {
        title: () =>
            `${
                configManager.getGlobalStorage('debug')
                    ? i18n.t('closeDebugMode')
                    : i18n.t('openDebugMode')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
            `${
                configManager.getGlobalStorage('debug')
    ...
    toggleGUIStatus: {
        title: () =>
            `${
                configManager.getGlobalStorage('ui.enable') === false
                    ? i18n.t('enableGUI')
                    : i18n.t('disableGUI')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setGlobalStorage(
                    'ui.enable',
                    !configManager.getGlobalStorage('ui.enable'),
                )
                window.location.reload()
            }
        },
    ...
    toggleGUIStatusUnderCurrentSite: {
        title: () =>
            `${
                configManager.getLocalStorage('ui.enable') === false
                    ? i18n.t('enableGUI')
                    : i18n.t('disableGUI')
            } 「${i18n.t('localSetting')}」`,
        desc: () =>
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setLocalStorage(
                    'ui.enable',
                    !configManager.getLocalStorage('ui.enable'),
                )
                window.location.reload()
            }
        },
    ...
    alwaysShowGraphicalInterface: {
        title: `${i18n.t('toggleStates')}${i18n.t(
            'alwaysShowGraphicalInterface',
        )} 「${i18n.t('globalSetting')}」`,
        desc: `${i18n.t('toggleStates')}${i18n.t(
            'alwaysShowGraphicalInterface',
        )} 「${i18n.t('globalSetting')}」`,
        fn: () => {
            ...
            const confirm = window.confirm(
                alwaysShow === true
                    ? `${i18n.t('disable')}${i18n.t(
                          'alwaysShowGraphicalInterface',
                      )} 「${i18n.t('globalSetting')}」`
                    : `${i18n.t('alwaysShowGraphicalInterface')} 「${i18n.t(
                          'globalSetting',
                      )}」`,
            )
            if (confirm) {
    ...
    toggleHotkeysStatus: {
        title: () =>
            `${
                configManager.getGlobalStorage('enableHotkeys') === false
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setGlobalStorage(
                    'enableHotkeys',
                    !configManager.getGlobalStorage('enableHotkeys'),
                )
                window.location.reload()
            }
        },
    ...
    toggleHotkeysStatusUnderCurrentSite: {
        title: () =>
            `${
                configManager.getLocalStorage('enableHotkeys') === false
                    ? i18n.t('enableHotkeys')
                    : i18n.t('disableHotkeys')
            } 「${i18n.t('localSetting')}」`,
        desc: () =>
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setLocalStorage(
                    'enableHotkeys',
                    !configManager.getLocalStorage('enableHotkeys'),
                )
                window.location.reload()
            }
        },
    ...
    toggleMouseControl: {
        title: () =>
            `${
                configManager.getGlobalStorage('mouse.enable') === false
                    ? i18n.t('mouse.enable')
                    : i18n.t('mouse.disable')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
            `${
                configManager.getGlobalStorage('mouse.enable') === false
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setGlobalStorage(
                    'mouse.enable',
                    !configManager.getGlobalStorage('mouse.enable'),
                )
                window.location.reload()
            }
        },
    ...
    toggleMouseControlUnderCurrentSite: {
        title: () =>
            `${
                configManager.getLocalStorage('mouse.enable') === false
                    ? i18n.t('mouse.enable')
                    : i18n.t('mouse.disable')
            } 「${i18n.t('localSetting')}」`,
        desc: () =>
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setLocalStorage(
                    'mouse.enable',
                    !configManager.getLocalStorage('mouse.enable'),
                )
                window.location.reload()
            }
        },
    ...
    setMouseLongPressTime: {
        ...
        fn: () => {
            const longPressTime = prompt(
                `${i18n.t('mouse.longPressTime')}「${i18n.t(
                    'globalSetting',
                )}」`,
                configManager.getGlobalStorage('mouse.longPressTime') || 600,
            )
            if (longPressTime) {
                configManager.setGlobalStorage(
                    'mouse.longPressTime',
                    Number(longPressTime),
                )
                window.location.reload()
            }
        },
    ...
    toggleDownloadControl: {
        title: () =>
            `${
                configManager.getGlobalStorage('download.enable') === false
                    ? i18n.t('mediaDownload.enable')
                    : i18n.t('mediaDownload.disable')
            } 「${i18n.t('globalSetting')}」`,
        desc: () =>
            `${
                configManager.getGlobalStorage('download.enable') === false
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setGlobalStorage(
                    'download.enable',
                    !configManager.getGlobalStorage('download.enable'),
                )
                window.location.reload()
            }
        },
    ...
    toggleDownloadControlUnderCurrentSite: {
        title: () =>
            `${
                configManager.getLocalStorage('download.enable') === false
                    ? i18n.t('mediaDownload.enable')
                    : i18n.t('mediaDownload.disable')
            } 「${i18n.t('localSetting')}」`,
        desc: () =>
        ...
        fn: () => {
            ...
            if (confirm) {
                configManager.setLocalStorage(
                    'download.enable',
                    !configManager.getLocalStorage('download.enable'),
                )
                window.location.reload()
            }
        },
    ...
    cleanRemoteHelperInfo: {
        ...
        fn: () => {
            ...
            configManager.setGlobalStorage(
                'contactRemoteHelperSuccessTime',
                false,
            )
...
function proxyHTMLMediaElementEvent() {
    ...
    HTMLMediaElement.prototype._rawAddEventListener_ =
        HTMLMediaElement.prototype.addEventListener
    HTMLMediaElement.prototype._rawRemoveEventListener_ =
        HTMLMediaElement.prototype.removeEventListener

    HTMLMediaElement.prototype.addEventListener = new Proxy(
        HTMLMediaElement.prototype.addEventListener,
        {
            apply(target, ctx, args) {
                        ...
                        apply(target, ctx, args) {
                            if (ctx) {
                                ...
                                if (
                                     ...
                                     * 其他检测手段待补充
                                     */
                                    const blockRatechangeBehave1 =
                                        oldRate !== ctx.playbackRate ||
                                        Date.now() - startTime > 1000
                                    const blockRatechangeBehave2 =
                                        ctx._setPlaybackRate_ &&
                                        ctx._setPlaybackRate_.value !==
                                            ctx.playbackRate
                                    if (
                                        blockRatechangeBehave1 ||
                                        blockRatechangeBehave2
                                    ) {
                                        debug.info(
                                            `[execVideoEvent][${eventName}]检测到可能存在阻止调速的行为，已禁止${eventName}事件的执行`,
                                            listener,
                                        )
...
const mediaSource = (function () {
    ...
    function connectMediaSourceWithMediaElement(mediaEl) {
        ...
        mediaSourceMap.forEach((mediaSourceInfo) => {
            if (
                mediaSourceInfo.mediaSource.__objURL__ &&
                curSrc === mediaSourceInfo.mediaSource.__objURL__
            ) {
    ...
    function cleanMediaSourceData() {
        function removeMediaSourceData(mediaSourceInfo) {
            console.log(
                '[cleanMediaSourceData][removeMediaSourceData]',
                mediaSourceInfo.mediaUrl ||
                    mediaSourceInfo.mediaSource.__objURL__,
            )
            original.map.delete.call(
                mediaSourceMap,
        ...
        mediaSourceMap.forEach((mediaSourceInfo) => {
            if (
                !mediaSourceInfo.mediaElement ||
                !(mediaSourceInfo.mediaElement instanceof HTMLMediaElement)
            ) {
    ...
    function proxyMediaSourceMethod() {
        ...
        originURLMethods.createObjectURL =
            originURLMethods.createObjectURL ||
            URL.prototype.constructor.createObjectURL
        URL.prototype.constructor.createObjectURL = new original.Proxy(
            originURLMethods.createObjectURL,
            {
                apply(target, ctx, args) {
                    const object = args[0]
                    ...
                    if (
                        ...
                        !original.map.has.call(objectURLMap, object)
                    ) {
        ...
        MediaSource.prototype.addSourceBuffer = new original.Proxy(
            originMethods.addSourceBuffer,
            {
                apply(target, ctx, args) {
                    ...
                    const mediaSourceInfo = original.map.get.call(
                        mediaSourceMap,
                        ctx,
                    )
                    ...
                    try {
                        ...
                        mediaInfo.codecs = tmpArr[1]
                            .trim()
                            .replace('codecs=', '')
                            .replace(/["']/g, '')
                    } catch (e) {
                        original.console.error(
                            '[addSourceBuffer][mediaInfo] 媒体信息解析出错',
                            sourceBufferItem,
                            e,
                        )
                    }
                    ...
                    sourceBuffer.appendBuffer = new original.Proxy(
                        sourceBufferItem.originAppendBuffer,
                        {
                            apply(bufTarget, bufCtx, bufArgs) {
                                ...
                                if (original.map.get.call(objectURLMap, ctx)) {
                                    mediaSourceInfo.mediaUrl =
                                        original.map.get.call(objectURLMap, ctx)
                                }

                                /* 如果appendBuffer依然活跃，但对应的mediaSource却被清理了，则尝试重新将数据关联回去 */
                                if (
                                    !original.map.get.call(mediaSourceMap, ctx)
                                ) {
                                    original.map.set.call(
                                        mediaSourceMap,
                                        ctx,
                                        mediaSourceInfo,
                                    )
        ...
        MediaSource.prototype.endOfStream = new original.Proxy(
            originMethods.endOfStream,
            {
                apply(target, ctx, args) {
                    /* 标识当前媒体流已加载完成 */
                    const mediaSourceInfo = original.map.get.call(
                    ...
                    if (mediaSourceInfo) {
                        ...
                        if (
                            mediaSourceInfo.mediaElement &&
                            mediaSourceInfo.autoDownload &&
                            !mediaSourceInfo.hasDownload
                        ) {
    ...
    function downloadMediaSource(mediaEl, title) {
        ...
        mediaSourceMap.forEach((mediaSourceInfo) => {
            ...
            if (mediaSourceInfo.hasDownload) {
                const confirm = original.confirm(
                    i18n.t('mediaDownload.hasDownload'),
                )
                if (!confirm) {
                    return false
                }
            ...
            if (!mediaSourceInfo.hasDownload && !mediaSourceInfo.endOfStream) {
                ...
                const confirm = original.confirm(
                    i18n.t('mediaDownload.notEndOfStream'),
                )
                if (!confirm) {
                    if (mediaSourceInfo.autoDownload) {
                        const cancelAutoDownload = original.confirm(
                            i18n.t('mediaDownload.cancelAutoDownload'),
                        )
                        if (cancelAutoDownload) {
                            mediaSourceInfo.autoDownload = false
                        }
                    } else {
                        const autoDownload = original.confirm(
                            i18n.t('mediaDownload.autoDownload'),
                        )
                        if (autoDownload) {
            ...
            mediaSourceInfo.sourceBuffer.forEach((sourceBufferItem) => {
                if (
                    !sourceBufferItem.mimeCodecs ||
                    sourceBufferItem.mimeCodecs.toString().indexOf(';') === -1
                ) {
                    const msg =
                        '[downloadMediaSource][mimeCodecs][error] mimeCodecs不存在或信息异常，无法下载'
                ...
                try {
                    let mediaTitle = `${
                        mediaSourceTitle ||
                        sourceBufferItem.mediaInfo.title ||
                        title ||
                    ...
                    if (
                        ...
                        mediaTitle = original.prompt(
                            i18n.t('mediaDownload.confirmTitle'),
                            mediaTitle,
                        )
                    ...
                    a.href = URL.createObjectURL(
                        new Blob(sourceBufferItem.bufferData),
                    )
...
const combinationKeysMonitor = (function () {
    ...
    function init(win = window) {
        ...
        function activeCombinationKeysState(event) {
            isModifierKey(event.code) &&
                combinationKeysState.set(event.code, true)
        }
        ...
        function inactivateCombinationKeysState(event) {
            ...
            if (isModifierKey(event.code)) {
                ...
                timers[event.code] = setTimeout(() => {
                    combinationKeysState.set(event.code, false)
                }, 50)
            }
        }
...
class HotkeysRunner {
    ...
    isMatch(event, press) {
        ...
        const combinationKeys =
            event.combinationKeys || combinationKeysMonitor.getCombinationKeys()
        const mods = press[0]
        const key = press[1]
    ...
    isMatchPrevPress(press) {
        return this.isMatch(this.prevPress, press)
    }
    ...
    run(opts = {}) {
        ...
        const KeyboardEventList = this.windowList.map(
            (win) => win.KeyboardEvent,
        )
        ...
        this.hotkeys.forEach((hotkeyConf) => {
            ...
            if (
                this.prevPress &&
                (hotkeyConf.keyBindings.length <= 1 ||
                    !this.isMatchPrevPress(press))
            ) {
                return false
            }
            ...
            const stopPropagation =
                opts.stopPropagation || hotkeyConf.stopPropagation
            const preventDefault =
                opts.preventDefault || hotkeyConf.preventDefault
            stopPropagation && event.stopPropagation()
            preventDefault && event.preventDefault()

            /* 记录上一次操作的快捷键，且一段时间后清空该操作的记录 */
            if (
                press === hotkeyConf.keyBindings[0] &&
                hotkeyConf.keyBindings.length > 1
            ) {
                /* 将prevPress变成一个具有event相关字段的对象 */
                this.prevPress = {
                    combinationKeys:
                        combinationKeysMonitor.getCombinationKeys(),
                    code: event.code,
                    key: event.key,
            ...
            if (!(commandFunc instanceof Function) && target) {
                throw new Error(
                    `[hotkeysRunner] 未找到command: ${hotkeyConf.command} 对应的函数`,
                )
            }

            if (hotkeyConf.when && conditionHandler instanceof Function) {
                const isMatchCondition = conditionHandler.apply(
                    target,
                    toArrArgs(hotkeyConf.when),
                )
...
function MediaElementAmplifier(mediaElem) {
    ...
    this._source = this._context.createMediaElementSource(
        (this._element = mediaElem),
    )
...
;[
    ...
    [
        ...
        function (opt_getPerceived) {
            return (
                (opt_getPerceived ? this.getLoudness() : 1) *
                this._element.volume
            )
        },
    ],
    [
    ...
    [
        'getDecibels',
        function () {
            return 20 * Math.log10(this.getGainValue())
        },
    ],
    [
        'setDecibels',
        function (value) {
            this.setGainValue(Math.pow(10, value / 20))
        },
    ],
    [
        'getLoudness',
        function () {
            return Math.pow(2, this.getDecibels() / 10)
        },
    ],
    [
        'setLoudness',
        function (value) {
            this.setDecibels(10 * Math.log2(value))
        },
    ],
].forEach(function (name, fn) {
    if (typeof name === 'string') {
        fn = function () {
            return this[name.replace('get', '').toLowerCase()]
        }
    } else {
        fn = name[1]
        name = name[0]
...
function mediaDownload(mediaEl, title, downloadType) {
    ...
    if (mediaEl && mediaUrl && !mediaUrl.startsWith('blob:')) {
        ...
        let mediaTitle = `${
            ...
            document.title ||
            Date.now()
        }_${mediaInfo.type}.${mediaInfo.format}`
        ...
        if (downloadType === 'blob' || mediaEl.duration < 60 * 5) {
            if (mediaState.downloading) {
                ...
                if (Date.now() - mediaState.downloading < 1000 * 1) {
                    ...
                    const confirm = original.confirm(
                        i18n.t('mediaDownload.downloading'),
                    )
                    if (!confirm) {
                        return false
                    }
...
const remoteHelper = {
    init() {
        ...
        if (!configManager.isGlobalStorageUsable()) {
            return false
        }

        const contactRemoteHelperSuccessTime = configManager.getGlobalStorage(
            'contactRemoteHelperSuccessTime',
        )
        let lastContactRemoteHelperTime = configManager.getGlobalStorage(
            'lastContactRemoteHelperTime',
        )
         ...
         */
        const syncInterval =
            configManager.getGlobalStorage('remoteHelperSyncInterval') ||
            1000 * 60 * 60 * 12
        if (
            contactRemoteHelperSuccessTime &&
            Date.now() - contactRemoteHelperSuccessTime < syncInterval
        ) {
            return false
        }
        if (Date.now() - lastContactRemoteHelperTime < 1000 * 60) {
            return false
        }
    ...
    establishRemoteConnection() {
        const lastSucTime =
            configManager.getGlobalStorage('contactRemoteHelperSuccessTime') ||
            '0'
        const timeStr =
        ...
        iframe.style.cssText =
            'width:0; height:0; border:none; visibility:hidden; opacity:0;'
        const insertIframe = () => {
        ...
        if (!document.body || !document.body.appendChild) {
            window.addEventListener('DOMContentLoaded', insertIframe, {
                once: true,
            })
        } else {
            insertIframe()
    ...
    async remoteHandler() {
        if (
            !location.href.startsWith(remoteHelperUrl) ||
            !configManager.isGlobalStorageUsable()
        ) {
            return false
        }
        ...
        function syncRemoteData(pageWindow) {
            ...
            if (pageWindow.remoteVersion) {
                configManager.setGlobalStorage(
                    'remoteVersion',
                    pageWindow.remoteVersion,
                )
            }

            if (pageWindow.remoteHelperSyncInterval) {
                configManager.setGlobalStorage(
                    'remoteHelperSyncInterval',
                    pageWindow.remoteHelperSyncInterval,
                )
            }
...
function registerMouseEvent(h5player) {
    ...
    document.addEventListener(
        ...
        function (event) {
            ...
            if (!isCoordinateInElement(event.clientX, event.clientY, player)) {
                return
            }
...
const h5playerUI = function (window) {
    var h5playerUI = (function () {
        ...
        const r$6 = (t) =>
                new n$5('string' == typeof t ? t : t + '', void 0, s$3),
            i$3 = (t, ...e) => {
                const o =
                    1 === t.length
                        ? t[0]
        ...
        ;(b.elementStyles = []),
            ...
            (a$1.reactiveElementVersions ??= []).push('2.0.3')
        ...
        const t$1 = globalThis,
            ...
            s$2 = i$1
                ? i$1.createPolicy('lit-html', { createHTML: (t) => t })
                : void 0,
            e$6 = '$lit$',
            h$2 = `lit$${(Math.random() + '').slice(9)}$`,
            o$3 = '?' + h$2,
            n$3 = `<${o$3}>`,
            r$4 = document,
            l = () => r$4.createComment(''),
        ...
        const P = (t, i) => {
            ...
            for (let i = 0; i < s; i++) {
                ...
                for (
                    ;
                    y < s.length &&
                    ((c.lastIndex = y), (u = c.exec(s)), null !== u);
                ...
                l +=
                    c === f$2
                        ...
                        : d >= 0
                        ? (o.push(a),
                          s.slice(0, d) + e$6 + s.slice(d) + h$2 + x)
                        : s + h$2 + (-2 === d ? i : x)
            }
                ...
                for (; null !== (r = E.nextNode()) && d.length < u; ) {
                    if (1 === r.nodeType) {
                        ...
                        if (r.data === o$3) d.push({ type: 2, index: c })
                            ...
                            let t = -1
                            for (; -1 !== (t = r.data.indexOf(h$2, t + 1)); )
                                d.push({ type: 7, index: c }),
                                    (t += h$2.length - 1)
                        }
                    c++
                }
        ...
        class M {
            ...
            get parentNode() {
                ...
                const i = this._$AM
                return (
                    void 0 !== i && 11 === t?.nodeType && (t = i.parentNode), t
                )
            }
            get startNode() {
                return this._$AA
            ...
            _(t) {
                this._$AH !== T && c$1(this._$AH)
                    ? (this._$AA.nextSibling.data = t)
                    : this.$(r$4.createTextNode(t)),
                    (this._$AH = t)
            }
            g(t) {
        ...
        Z?.(V, M), (t$1.litHtmlVersions ??= []).push('3.1.1')
        const j = (t, i, s) => {
...
`
         ...
         * SPDX-License-Identifier: BSD-3-Clause
         */ function r$1(r) {
            return n$2({ ...r, state: !0, attribute: !1 })
        }
        ...
        var ShoelaceElement = class extends s$1 {
            ...
            static define(name, elementConstructor = this, options = {}) {
                ...
                if (
                    'version' in elementConstructor &&
                    elementConstructor.version
                ) {
                ...
                if (
                    ...
                    existingVersion =
                        ' v' + currentlyRegisteredConstructor.version
                }
                if (
                    newVersion &&
                    existingVersion &&
        ...
        async function detectOverflow(state, options) {
            ...
            const clippingClientRect = rectToClientRect(
                await platform.getClippingRect({
                    element: (
                        (_await$platform$isEle = await (platform.isElement ==
                        null
                            ? void 0
        ...
        const size$1 = function (options) {
            ...
            return {
                ...
                async fn(state) {
                    ...
                    const { apply = () => {}, ...detectOverflowOptions } =
                        evaluate(options, state)
                    const overflow = await detectOverflow(
                        state,
                        detectOverflowOptions,
            ...
            class extends i {
                ...
                update(s, [i]) {
                    if (void 0 === this.it) {
                        ...
                        for (const t in i)
                            i[t] && !this.st?.has(t) && this.it.add(t)
                        return this.render(i)
                    }
                    const r = s.element.classList
                    for (const t of this.it)
                        t in i || (r.remove(t), this.it.delete(t))
                    for (const t in i) {
                        const s = !!i[t]
                        s === this.it.has(t) ||
                            this.st?.has(t) ||
                ...
                computePosition(this.anchorEl, this.popup, {
                    ...
                    if (this.arrow) {
                        ...
                        if (this.arrowPlacement === 'start') {
                            ...
                            left =
                                typeof arrowX === 'number' ? `${arrowX}px` : ''
                            top =
                                typeof arrowY === 'number' ? `${arrowY}px` : ''
                        }
    ...
    `
        ...
        __decorateClass([n$2({ type: Boolean })], SlPopup.prototype, 'arrow', 2)
        __decorateClass(
        ...
        __decorateClass([n$2({ type: Boolean })], SlPopup.prototype, 'flip', 2)
        __decorateClass(
            [
                n$2({
        ...
        __decorateClass([n$2({ type: Boolean })], SlPopup.prototype, 'shift', 2)
        __decorateClass(
            [n$2({ type: Object })],
            SlPopup.prototype,
            'shiftBoundary',
...
`
        ...
        function getTabbableBoundary(root) {
            ...
            const end =
                (_b = tabbableElements[tabbableElements.length - 1]) != null
                    ? _b
                    : null
            return { start, end }
        }
            ...
            function walk(el) {
                if (el instanceof Element) {
                    ...
                    if (
                        ...
                        slotChildrenOutsideRootElement(el)
                    ) {
        ...
        function animateTo(el, keyframes, options) {
            return new Promise((resolve) => {
                if (
                    (options == null ? void 0 : options.duration) === Infinity
                ) {
            ...
            translation.map((t) => {
                ...
                if (translations.has(code)) {
                    translations.set(
                        code,
                        Object.assign(
                            Object.assign({}, translations.get(code)),
                            t,
                        ),
                    )
                } else {
        ...
        let LocalizeController$1 = class LocalizeController {
            ...
            exists(key, options) {
                ...
                const { primary, secondary } = this.getTranslationData(
                    (_a = options.lang) !== null && _a !== void 0
                        ? _a
                        : this.lang(),
                )
        ...
        var SlDropdown = class extends ShoelaceElement {
            ...
            focusOnTrigger() {
                ...
                if (
                    typeof (trigger == null ? void 0 : trigger.focus) ===
                    'function'
                ) {
                    trigger.focus()
                }
...
`
    ...
    `
        ...
        __decorateClass([e$4('slot')], SlMenu.prototype, 'defaultSlot', 2)
...
`
         ...
         * SPDX-License-Identifier: BSD-3-Clause
         */ const e$1 = (o, t) =>
                void 0 === t ? void 0 !== o?._$litType$ : o?._$litType$ === t,
        ...
        class f extends i {
            ...
            _$AO(i, t = !0) {
                i !== this.isConnected &&
                    ((this.isConnected = i),
                    i ? this.reconnected?.() : this.disconnected?.()),
                    t && (s(this, i), o$1(this))
            }
            setValue(t) {
                ...
                class extends f {
                    ...
                    update(i, [s]) {
                        ...
                        return (
                            ...
                            (e || this.rt !== this.lt) &&
                                ((this.G = s),
                                (this.ct = i.options?.host),
                                this.ot((this.lt = i.element))),
                            T
                        )
                    }
                    ot(t) {
                    ...
                    get rt() {
                        return 'function' == typeof this.G
                            ? o.get(this.ct ?? globalThis)?.get(this.G)
                            : this.G?.value
                    }
                    disconnected() {
        ...
        var SubmenuController = class {
            ...
            handleSubmenuEntry(event) {
                ...
                for (const elt of submenuSlot.assignedElements()) {
                    menuItems = elt.querySelectorAll(
                        "sl-menu-item, [role^='menuitem']",
                    )
                    if (menuItems.length !== 0) {
                        break
                    }
            ...
            updateSkidding() {
                var _a
                if (
                    !((_a = this.host.parentElement) == null
                        ? void 0
                        : _a.computedStyleMap)
                ) {
                    return
  ...
  `,
        ...
        <use part="use" href="${url}"></use>
            ...
            async setIcon() {
                ...
                const library = fromLibrary
                    ? getIconLibrary(this.library)
                    : void 0
                if (!url) {
                    this.svg = null
                    return
                ...
                switch (svg) {
                    ...
                    default:
                        ...
                        ;(_a = library == null ? void 0 : library.mutator) ==
                        null
                            ? void 0
        ...
        __decorateClass([r$1()], SlIcon.prototype, 'svg', 2)
        __decorateClass([n$2({ reflect: true })], SlIcon.prototype, 'name', 2)
    ...
    `
        ...
        __decorateClass([n$2()], SlMenuItem.prototype, 'type', 2)
        __decorateClass(
            [n$2({ type: Boolean, reflect: true })],
            SlMenuItem.prototype,
            'checked',
        ...
        __decorateClass([n$2()], SlMenuItem.prototype, 'value', 2)
        __decorateClass(
            [n$2({ type: Boolean, reflect: true })],
            SlMenuItem.prototype,
            'disabled',
...
`
                ...
                dropdownMenu: [
                    ...
                    },
                    {
                        ...
                        subMenu: [
                            ...
                            {
                                title: `${i18n.t('languageSettings')}「${i18n.t(
                                    'globalSetting',
                                )}」`,
                                desc: `${i18n.t('languageSettings')}「${i18n.t(
                                    'globalSetting',
                                )}」`,
                                disabled: !isGlobalStorageUsable,
                                subMenu: [
        ...
        function menuConfigPreprocess(menuConfig, refDom) {
            ...
            return menuConfig.map((item) => {
                if (item.dropdownMenu) {
                    item.dropdownMenu = menuConfigPreprocess(
                        item.dropdownMenu,
                        refDom,
                    )
                }
        ...
        function convertDropdownMenuToTemplate(
            dropdownMenu,
            isRootMenu = true,
        ) {
            const menuItems = dropdownMenu
                .map((item) => {
                    ...
                    if (item.subMenu) {
                        return `
        <sl-menu-item class="h5p-menu-action" value="${id}" title="${
                            desc || title
                        }" data-action="${
                            item.action || ''
                        }" data-args='${JSON.stringify(item.args || null)}'>
          ${title}
      ...
      `
                        ...
                        return `<sl-menu-item class="h5p-menu-action" value="${id}" title="${
                            desc || title
                        }" data-action="${
                            item.action || ''
  ...
  <div class="h5p-action-mod">
      ${menuConfig
          .map((item) => {
              ...
              const title =
                  (item.title instanceof Function
                      ...
                      : item.title) || ''
              const desc =
                  (item.desc instanceof Function ? item.desc() : item.desc) ||
                  ''
              const iconHtml = item.icon
                  ? `<sl-icon src="${item.icon}"></sl-icon>`
                  : ''
              const menuDesc =
                  item.iconOnly && iconHtml
                      ? ''
                      : `<span class="h5p-desc">${title}</span>`

              if (item.dropdownMenu) {
                  return `
            <sl-dropdown distance="6">
              <span slot="trigger" class="h5p-action-btn" title="${
                  desc || title
              }" data-title="${title}" data-action="${item.action || ''}">
                ${iconHtml}
                ${menuDesc}
              </span>
          ...
          `
                  ...
                  return `
            <span class="h5p-action-btn h5p-menu-action" title="${
                desc || title
            }" data-title="${title}" data-action="${
                      item.action || ''
                  }" data-args='${JSON.stringify(item.args || null)}'>
              ${iconHtml}
              ${menuDesc}
  ...
  `
        ...
        function createRecommendModTemplate(refDom) {
            const showMod =
                isGlobalStorageUsable &&
                configManager$1.getGlobalStorage('ui.mod.recommend.enable')
            if (!showMod) {
                return ''
            }
            ...
            let recommendList =
                configManager$1.getGlobalStorage('recommendList') ||
            ...
            recommendList = recommendList.sort(
                (a, b) => (b.priority || 0) - (a.priority || 0),
            )

            const recommendHtml = recommendList
                .map((item) => {
        ...
        function registerRecommendModToggle(recommendWrap, reRender) {
            if (
                !reRender &&
                (!recommendWrap || recommendWrap.__h5pRecommendModRegistered__)
            ) {
                return
            }
            ...
            const toggleRecommend = () => {
                if (recommendWrap.__stopToggle__) {
                    return
                }
                const recommendItems = recommendWrap.querySelectorAll(
                    '.h5p-recommend-item',
                )
            ...
            recommendWrap.__h5pRecommendModInterval__ = setInterval(
                toggleRecommend,
                3000,
            )
            if (!reRender) {
                recommendWrap.addEventListener('mouseenter', () => {
                    recommendWrap.__stopToggle__ = true
                })
                recommendWrap.addEventListener('mouseleave', () => {
                    recommendWrap.__stopToggle__ = false
                })
            }
        ...
        function menuActionHandler(obj) {
            ...
            if (
                action &&
                (h5Player[action] instanceof Function ||
                    globalFunctional[action])
            ) {
                ...
                try {
                    if (action === 'setPlaybackRate') {
                        ...
                        globalFunctional[action] &&
                        globalFunctional[action].fn instanceof Function
                    ) {
        ...
        if (!window.h5playerUIProvider) {
            throw new Error(
                'h5playerUIProvider is not defined, please check if you have imported h5playerUIProvider.js',
            )
        }
        ...
        const h5playerUI = {
            ...
            popup(element, h5Player) {
                if (
                    this.__disableGUITemporarily__ ||
                    element.__disableGUITemporarily__
                ) {
                    return false
                }
                ...
                /* 防止popup渲染过于频繁 */
                if (
                    this.lastRenderedPopupTime &&
                    Date.now() - this.lastRenderedPopupTime < 100
                ) {
                    return false
                } else {
                ...
                /* 防止popup死循环渲染 */
                if (
                    element.__popupRenderedCount__ &&
                    element.__popupRenderedCount__ > 15
                ) {
                    return false
                } else {
                    element.__popupRenderedCount__ =
                        element.__popupRenderedCount__
                            ? element.__popupRenderedCount__ + 1
                            : 1
                }
                ...
                const menuTemplate = createMenuTemplate(
                    menuConfigPreprocess(menuConfig, element),
                )
                popupWrap = parseHTML(
                    `
      <div id="${popupWrapId}" class="h5player-popup-wrap">
        ...
        <div class="h5player-popup-content">
          ...
          <div class="h5p-recommend-wrap">
            <div style="overflow:hidden">${createRecommendModTemplate(
                element,
            )}</div>
          </div>
    ...
    `,
                ...
                setTimeout(() => {
                    registerRecommendModToggle(
                        popupWrap.querySelector('.h5p-recommend-wrap'),
                    )
                }, 100)
                ...
                function reRenderMenuMod() {
                    const menuWrap = popupWrap.querySelector(
                        '.h5player-popup-content .h5p-menu-wrap',
                    )
                ...
                function reRenderRecommendMod() {
                    const recommendWrap = popupWrap.querySelector(
                        '.h5player-popup-content .h5p-recommend-wrap',
                    )
                    const recommendMod = popupWrap.querySelector(
                        '.h5player-popup-content .h5p-recommend-wrap>div',
                    )
                    if (recommendWrap && recommendMod) {
                        recommendWrap.removeChild(recommendMod)

                        const newRecommendModTemplate = `<div style="overflow:hidden">${createRecommendModTemplate(
                            element,
                        )}</div>`
                ...
                const alwaysShowUIBar =
                    configManager.getGlobalStorage('ui.alwaysShow')
                ...
                popupWrap.addEventListener('mouseleave', () => {
                    ...
                    if (isOutOfDocument(element)) {
                        ...
                        mouseleaveTimer = setTimeout(() => {
                            ...
                            const dropdowns =
                                popupWrap.querySelectorAll('sl-dropdown')
                ...
                async function openDropdown(event) {
                    ...
                    if (
                        !(
                            target.classList.contains(actionBtnClass) ||
                    ...
                    const isDropdownDom =
                        curDropdown &&
                        curDropdown.tagName.toLowerCase() === 'sl-dropdown'
...
const h5Player = {
    ...
    getPlayerWrapDom: function () {
        ...
        eachParentNode(player, function (parent) {
            ...
            if (parentBox.width && parentBox.height) {
                if (
                    parentBox.width === playerBox.width &&
                    parentBox.height === playerBox.height
                ) {
                    wrapDom = parent
                }
            }
    ...
    initPlayerInstance(isSingle) {
        ...
        if (!player._hasPlayerInitEvent_) {
            ...
            player.addEventListener('playing', function (event) {
                ...
                /* 同步播放音量 */
                if (
                    configManager.get('enhance.blockSetVolume') === true &&
                    event.target.muted === false
                ) {
                    t.setVolume(
                        configManager.getGlobalStorage('media.volume'),
                        true,
                    )
                }
        ...
        function updataBufferedTime(player) {
            ...
            if (player.buffered.length > 0) {
                const bufferedTime = player.buffered.end(
            ...
            if (
                t.autoGotoBufferedTime &&
                player.bufferedTime &&
                t.player() === player &&
                player.bufferedTime < player.duration - 1 &&
                player.currentTime < player.bufferedTime - 1
            ) {
        ...
        needInitEvent &&
            player.addEventListener('loadeddata', function () {
                debug.log(
                    `[player][loadeddata] ${player.src} video duration: ${player.duration} video dom:`,
                    player,
                )
                srcRecord(player)
            })
        ...
        needInitEvent &&
            player.addEventListener('timeupdate', function () {
                ...
                if (
                    Date.now() - lastRegisterUIPopupTime > 800 &&
                    tryRegisterUIPopupCount < 60
                ) {
    ...
    isLeavepictureinpictureAwhile() {
        const t = this
        return (
            t.leavepictureinpictureTime &&
            Date.now() - t.leavepictureinpictureTime < 1000 * 10
        )
    },
            ...
            if (Reflect.has(player, key) && !Reflect.has(player, originKey)) {
                ...
                const proxy = new Proxy(player[key], {
                    apply(target, ctx, args) {
                        ...
                        const hangUpDetail =
                            hangUpInfo[key] || hangUpInfo['hangUp_' + key]
                        const needHangUp =
                            hangUpDetail && hangUpDetail.timeout >= Date.now()
                        if (needHangUp) {
    ...
    initAutoPlay: function (p) {
        ...
        if (taskConf.autoPlay) {
            ...
            addMenu({
                title: () =>
                    configManager.getLocalStorage('media.autoPlay')
                        ? i18n.t('disableInitAutoPlay')
                        : i18n.t('enableInitAutoPlay'),
                fn: () => {
                    const confirm = window.confirm(
                        configManager.getLocalStorage('media.autoPlay')
                            ? i18n.t('disableInitAutoPlay')
        ...
        if (
            !configManager.get('media.autoPlay') ||
            (!p && t.hasInitAutoPlay) ||
            !player ||
            (p && p !== t.player()) ||
            document.hidden
    ...
    getPlaybackRate() {
        let playbackRate =
            configManager.get('media.playbackRate') || this.playbackRate
        if (isInIframe()) {
            const globalPlaybackRate =
                configManager.getGlobalStorage('media.playbackRate')
    ...
    setPlaybackRate: function (num, notips, duplicate, skipLock) {
        ...
        try {
            ...
            originalMethods.Object.defineProperty.call(
                Object,
                player,
                'playbackRate',
                {
                    ...
                    get: function () {
                        ...
                        return (
                            curPlaybackRate ||
                            playbackRateDescriptor.get.apply(player, arguments)
                        )
                    },
                    set: function (val) {
                        ...
                        !Number.isInteger(player._blockSetPlaybackRateTips_) &&
                            (player._blockSetPlaybackRateTips_ = 0)

                        if (TCC.doTask('blockSetPlaybackRate')) {
                            player._blockSetPlaybackRateTips_++
                            player._blockSetPlaybackRateTips_ < 3 &&
                                debug.info(
                                    '调速能力已被自定义的调速任务进行处理',
                                )
                            return false
        ...
        if (
            !duplicate &&
            configManager.get('enhance.blockSetPlaybackRate') === true
        ) {
            ...
            t._setPlaybackRateDuplicate_ = setTimeout(
                duplicatePlaybackRate,
                600,
            )
            /* 600ms时重新触发无效的话，再来个1200ms后触发，如果是1200ms才生效，则调速生效的延迟已经非常明显了 */
            t._setPlaybackRateDuplicate2_ = setTimeout(
                duplicatePlaybackRate,
                1200,
            )
        }
    ...
    setPlaybackRatePlus: function (num) {
        ...
        if (Date.now() - t.playbackRatePlusInfo[num].time < 300) {
            t.playbackRatePlusInfo[num].value =
    ...
    isLockCurrentTime: function () {
        ...
        if (
            player &&
            player.currentTimeInfo &&
            player.currentTimeInfo.lockTimeout
        ) {
        ...
        try {
            ...
            originalMethods.Object.defineProperty.call(
                ...
                'currentTime',
                {
                    ...
                    enumerable: true,
                    get: function () {
                        return currentTimeDescriptor.get.apply(
                            player,
                            arguments,
                        )
                    },
                    set: function (val) {
                        if (
                            typeof val !== 'number' ||
                            TCC.doTask('blockSetCurrentTime') ||
                            configManager.get('enhance.blockSetCurrentTime') ===
                                true
                        ) {
                            return false
                        ...
                        return currentTimeDescriptor.set.apply(
                            player,
                            arguments,
                        )
                    },
    ...
    setCurrentTimeUp(num, hideTips) {
        ...
        if (TCC.doTask('addCurrentTime'));
            ...
            if (this.player()) {
                ...
                if (!hideTips) {
                    this.tips(
                        i18n.t('tipsMsg.forward') +
                            num +
                            i18n.t('tipsMsg.seconds'),
                    )
                }
            }
        }
    ...
    setVolume: function (num, notips, outerCall) {
        ...
        if (num > 1 && player._amp_ && player._amp_.setLoudness) {
            ...
            !notips &&
                t.tips(i18n.t('tipsMsg.volume') + parseInt(num * 100) + '%')
            return true
        }
        ...
        !notips &&
            t.tips(
                i18n.t('tipsMsg.volume') + parseInt(player.volume * 100) + '%',
            )
    },
    ...
    collectTransformHistoryInfo() {
        ...
        Object.keys(t.defaultTransform).forEach((key) => {
            if (key === 'translate') {
                ...
                t.historyTransform.translate =
                    t.historyTransform.translate || {}
                Object.keys(translate).forEach((subKey) => {
                    if (
                        Number(t.translate[subKey]) !==
                        t.defaultTransform.translate[subKey]
                    ) {
                        t.historyTransform.translate[subKey] =
                            t.translate[subKey]
                    }
                })
            } else {
    ...
    setTransform(notTips) {
        ...
        let tipsMsg =
            i18n.t('tipsMsg.videozoom') + `${(scale * 100).toFixed(0)}%`
        if (translate.x) {
    ...
    resetTransform(notTips) {
        ...
        if (
            t.isSameAsDefaultTransform() &&
            Object.keys(t.historyTransform).length
        ) {
            /* 还原成历史记录中的Transform值 */
    ...
    toggleAutoGotoBufferedTime() {
        ...
        t.tips(
            t.autoGotoBufferedTime
                ? i18n.t('autoGotoBufferedTime')
                : i18n.t('disableAutoGotoBufferedTime'),
        )
    },
    ...
    isAllowRestorePlayProgress: function () {
        const allowRestoreVal = configManager.get(
            `media.allowRestorePlayProgress.${window.location.host}`,
        )
    ...
    tips: function (str) {
        ...
        if (!isAudio) {
            ...
            const oldPosition =
                parentNode.getAttribute('def-position') ||
                window.getComputedStyle(parentNode).position
            if (parentNode.getAttribute('def-position') === null) {
                parentNode.setAttribute('def-position', oldPosition || '')
            }
            if (
                ['static', 'inherit', 'initial', 'unset', ''].includes(
                    oldPosition,
                )
            ) {
    ...
    filter: {
        ...
        setup: function () {
            let view =
                'brightness({0}) contrast({1}) saturate({2}) hue-rotate({3}deg) blur({4}px)'
            for (let i = 0; i < 5; i++) {
    ...
    setFilter(item, num, isDown) {
        ...
        t.filter.setup()
        t.tips(
            i18n.t(`tipsMsg.${nameMap[item]}`) +
                parseInt(t.filter.key[item] * 100) +
                '%',
        )
    },
    ...
    getPlayProgress: function (player) {
        ...
        if (!player) {
            ...
            if (progressMap[keyName]) {
                ...
                if (
                    Number.isNaN(Number(player.duration)) ||
                    Number(progressMap[keyName].duration) !==
        ...
        function recorder(player) {
            player._playProgressTimer_ = setTimeout(function () {
                ...
                const isToShort =
                    ...
                    player.duration < 120
                const isLeave =
                    document.visibilityState !== 'visible' && player.paused
                ...
                if (list.length > 10) {
                    ...
                    list.forEach(function (keyName) {
                        progressMap[keyName] &&
                            progressMap[keyName].t &&
                            timeList.push(progressMap[keyName].t)
                    })
    ...
    setPlayProgress: function (player) {
        ...
        if (
            ...
            t._firstProgressRecord_ === progressKey
        ) {
    ...
    setPlayerInstance(el) {
        ...
        if (isVideoElement(el)) {
            ...
            const parentElInfo =
                elParentNode && elParentNode.getBoundingClientRect()
            if (
                elInfo &&
                elInfo.width > 200 &&
                parentElInfo &&
                parentElInfo.width > 200
            ) {
                t.playerInstance = el
                t.initPlayerInstance(false)
            }
            ...
            if (
                isAudioElement(t.playerInstance) ||
                (isVideoElement(t.playerInstance) &&
                    !t.playerInstance.isConnected)
            ) {
                t.playerInstance = el
                t.initPlayerInstance(false)
            }
    ...
    intersectionObserver: new IntersectionObserver(
        function (entries, observer) {
            ...
            entries.forEach((entrie) => {
                ...
                if (
                    entrie.intersectionRatio > tmpIntersectionRatio &&
                    entrie.intersectionRatio > 0.4
                ) {
                    ...
                    if (
                        oldPlayer &&
                        oldPlayer._intersectionInfo_ &&
                        tmpIntersectionRatio <
                            oldPlayer._intersectionInfo_.intersectionRatio
                    ) {
                        /* 新实例的视图范围比旧的小，则不切换实例 */
                        return
                    }
    ...
    bindFakeEvent() {
         ...
         */
        if (
            !crossTabCtl.hasOpenPictureInPicture() &&
            !t.hasCrossOriginVideoDetected
        ) {
        ...
        monkeyMsg.on(
            'globalKeydownEvent',
            async (name, oldVal, newVal, remote) => {
                if (remote) {
                    if (isInCrossOriginFrame()) {
                        /**
                        ...
                        if (
                            !newVal.originTab &&
                            (document.pictureInPictureElement ||
                                t.isLeavepictureinpictureAwhile())
                        ) {
                            triggerFakeEvent(name, oldVal, newVal, remote)
                        }
                    }
    ...
    setCustomConfiguration(config, tag = 'Default') {
        ...
        const configuration = configManager.mergeDefConf(
    ...
    init: function (global) {
        ...
        if (
            window.unsafeWindow &&
            window.unsafeWindow.__h5PlayerCustomConfiguration__
        ) {
            !t.hasExternalCustomConfiguration &&
                t.mergeExternalConfiguration(
                    window.unsafeWindow.__h5PlayerCustomConfiguration__,
                )
        }
        ...
        if (TCC && TCC.doTask('disable') === true) {
            debug.info(
                `[TCC][disable][${location.host}] 已禁止在该网站运行视频检测逻辑，您可查看任务配置中心的相关配置了解详情`,
            )
            return true
        }
        ...
        if (
            ...
            window.unsafeWindow.__setH5PlayerCustomConfiguration__ =
                t.mergeExternalConfiguration
        }
    },
}
...
async function h5PlayerInit() {
    ...
    const blackDomainList = configManager.get('blacklist.domains') || []
    const isInBlackList =
        blackUrlList.includes(location.href) ||
        blackDomainList.includes(location.host)

    if (isInBlackList) {
        console.warn(
            `[h5player][config][blacklist][${location.href}] \n当前页面已被加入黑名单，不执行h5player增强脚本的相关逻辑，如有需要开启，请在配置里的blacklist移除对应的地址`,
        )
    }
    ...
    try {
        if (isEnabled && !isInBlackList) {
            ...
            if (
                configManager.get('enhance.allowExperimentFeatures') &&
                configManager.get('download.enable')
            ) {
                mediaSource.init()
                debug.warn(
                    `[experimentFeatures][warning] ${i18n.t(
                        'experimentFeaturesWarning',
                    )}`,
                )
    ...
    if (!isEnabled || isInBlackList) {
        debug.warn(
            `[config][disable][${location.host}] 当前网站已禁用脚本，如要启用脚本，请在菜单里开启`,
        )
        return false
    }
    ...
    try {
        ...
        if (isInCrossOriginFrame()) {
            debug.log(
                '当前处于跨域受限的iframe中，h5Player部分功能可能无法正常开启',
    ...
    if (configManager.get('ui.enable') !== false) {
        if (window.customElements && document.adoptedStyleSheets) {
            ...
            debug.warn(
                '当前浏览器不支持customElements或adoptedStyleSheets，无法使用UI组件，建议使用Chrome 83+，Edge 83+',
            )
        }
    } else {
