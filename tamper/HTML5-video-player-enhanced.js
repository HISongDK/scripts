        ...
        mediaSourceMap.forEach((mediaSourceInfo) => {
            ...
            if (!mediaSourceInfo.hasDownload && !mediaSourceInfo.endOfStream) {
                ...
                original.console.log(
                    '[downloadMediaSource] 媒体数据还没完全就绪',
                    mediaSourceInfo,
                )
            }
            ...
            mediaSourceInfo.sourceBuffer.forEach((sourceBufferItem) => {
                ...
                try {
                    let mediaTitle =
                        ...
                        `${document.title || Date.now()}_${
                            sourceBufferItem.mediaInfo.type
                        }.${sourceBufferItem.mediaInfo.format}`

                    if (!sourceBufferItem.mediaInfo.title) {
                        mediaTitle =
                            original.prompt('请确认文件标题：', mediaTitle) ||
                            mediaTitle
                        sourceBufferItem.mediaInfo.title = mediaTitle
                    }

                    if (
                        !mediaTitle.endsWith(sourceBufferItem.mediaInfo.format)
                    ) {
                        mediaTitle =
                            mediaTitle + '.' + sourceBufferItem.mediaInfo.format
                    }
...
const taskConf = {
    ...
    'youtube.com': {
        ...
        afterPlay: function (h5Player, taskConf) {
            ...
            // NOTE: 空格或点击视频暂停后重新播放，字幕无法自动更新。手动下一秒时可以恢复，改一下 currentTime 规避这个问题
...
let monkeyMenuList = [
    ...
    },
    {
        ...
        fn: () => {
            openInTab(
                'https://h5player.anzz.top/home/Introduction.html#%E5%BF%AB%E6%8D%B7%E9%94%AE%E5%88%97%E8%A1%A8',
            )
        },
    },
    {
...
function mediaDownload(mediaEl, title, downloadType) {
    if (
        mediaEl &&
        (mediaEl.src || mediaEl.currentSrc) &&
        !mediaEl.src.startsWith('blob:')
    ) {
        const mediaInfo = {
        ...
        let mediaTitle = `${
            title || mediaEl.title || document.title || Date.now()
        ...
        if (downloadType === 'blob' || mediaEl.duration < 60 * 5) {
            if (mediaEl.downloading) {
                ...
                if (Date.now() - mediaEl.downloading < 1000 * 1) {
                    ...
                    const confirm =
                        original.confirm('文件正在下载中，确定重复执行此操作？')
                    if (!confirm) {
                        return false
                    }
                }
            ...
            mediaTitle =
                original.prompt('请确认文件标题：', mediaTitle) || mediaTitle
...
const h5Player = {
    ...
    initPlayerInstance(isSingle) {
        ...
        if (debug.isDebugMode()) {
            player.addEventListener('loadeddata', function () {
                debug.log(
                    `video url: ${player.src} video duration: ${player.duration} video dom:`,
                    player,
                )
            })
            ...
            originalMethods.Object.defineProperty.call(
                ...
                'currentTime',
                {
                    ...
                    set: function (val) {
                        if (
                            typeof val !== 'number' ||
                            TCC$1.doTask('blockSetCurrentTime') ||
                            configManager.get('enhance.blockSetCurrentTime') ===
                                true
                        ) {
                            return false
    ...
    setCurrentTimeUp(num, hideTips) {
        ...
        if (TCC$1.doTask('addCurrentTime'));
        else {
            if (this.player()) {
    ...
    setCurrentTimeDown(num) {
        ...
        if (TCC$1.doTask('subtractCurrentTime'));
        else {
            if (this.player()) {
