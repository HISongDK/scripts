...
let userID = 'e0abb66f-6f09-4feb-840a-035dc69006ab'
let proxyIP = ''
let sub = ''
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                if (路径 == '/') {
                    if (env.URL302) return Response.redirect(env.URL302, 302)
                        ...
                        return new Response(
                            JSON.stringify(request.cf, null, 4),
                            {
                                status: 200,
                                headers: {
                    ...
                    const vlessConfig = await 生成配置信息(
                        userID,
                        request.headers.get('Host'),
                        sub,
                        UA,
                ...
                if (new RegExp('/socks5=', 'i').test(url.pathname))
                    ...
                    if (socks5Address.includes('@')) {
                        ...
                        if (
                            base64Regex.test(userPassword) &&
                            !userPassword.includes(':')
                        )
                            userPassword = atob(userPassword)
                        socks5Address = `${userPassword}@${
                            socks5Address.split('@')[1]
                        }`
                    }
                }
                if (socks5Address) {
    ...
    if (sub) {
        ...
        if (hostName.includes('.workers.dev')) {
            addressesnotls = addressesnotls.concat(
                cfips.map(
                    (cidr) => generateRandomIPFromCIDR(cidr) + '#CF随机节点',
                ),
            )
        } else {
...
`
        ...
        let url = `${subProtocol}://${sub}/sub?host=${fakeHostName}&uuid=${
            fakeUserID + atob('JmVkZ2V0dW5uZWw9Y21saXUmcHJveHlpcD0=') + RproxyIP
        }`
        let isBase64 = true
            ...
            if ((!sub || sub == '') && isBase64 == true) {
                ...
                const response = await fetch(url, {
                    headers: {
                        'User-Agent':
                            UA +
                            atob('IENGLVdvcmtlcnMtZWRnZXR1bm5lbC9jbWxpdQ=='),
                    },
                })
            ...
            api.map((apiUrl) =>
                fetch(apiUrl, {
                    ...
                    headers: {
                        ...
                        'User-Agent': atob(
                            'Q0YtV29ya2Vycy1lZGdldHVubmVsL2NtbGl1',
                        ),
                    },
    ...
    if (noTLS == 'true') {
        ...
        notlsresponseBody = uniqueAddressesnotls
            .map((address) => {
                ...
                const vlessLink = `${协议类型}://${UUID}@${address}:${
                    port +
                    atob(
    ...
    const responseBody = uniqueAddresses
        .map((address) => {
            ...
            const vlessLink = `${协议类型}://${UUID}@${address}:${
                ...
                伪装域名
            }&fp=random&type=ws&host=${伪装域名}&path=${encodeURIComponent(
                最终路径,
...
function 生成动态UUID(密钥) {
    ...
    function 获取当前周数() {
        const 现在 = new Date()
        const 调整后的现在 = new Date(
            现在.getTime() + 时区偏移 * 60 * 60 * 1000,
        )
